...
//теперь поработаем с сервером

const inputRub = document.querySelector('#rub'),
	inputUsd = document.querySelector('#usd');

	inputRub.addEventListener('input', ()=>{
		const request = new new XMLHttpRequest(); //сформировали запрпос к серверу

		//request.open(); // (method[get, post, etc], url, async, login, password)
		request.opacity('GET', 'js/current.json');
		//запрос сформирован

		//теперь нужно доделать запрос и уточнить, а что именно мы отправляем
		// какая это информация, в чем она закодирована и тп
		//чтобы сервер точно понимал что принимает: изображение, или джейсон файл или еще че
		//для этой цели существуют http-заголовки
		request.setRequestHeader('Content-type', 'application/json; charset=utf-8');
		request.send(); //тк у нас метод гет, то тут пусто
		// если бы был пост, то тут была бы та инфа, что нужно скинуть на сервер				
		
		//после нашего запроса серверу и его последующего ответа мы получаем response
		//ответная реакция сервера
		//он нам присылает статус проделанной работы и инфу
		//это цифры. 200 и тд - это успешно. 
		//например, 400 и тд, в частности 404 - это ошибка "не найдено"
		//status

		//statusText - текст, что соответствует коду

		//response - ответ от сервера. Тот ответ, что задал разраб в бекэнде и что мы должны использовать на клиенте


		//readyState - статус состояния
		// от 0 до 4, 
		// 0 - объект создан (метод опен еще не вызывался)
		// 1 - метод опен был вызван
