...
//
/////////////////////
// Работа с событиями в мобильном телефоне
// 
// Самое важное - это нет кликов мыши!
// есть тапы (нажатия по экрану)
// (вообще, мобильны браузеры сами по себе неплохо справляются. Т.е. например на десктопе был клик, а в мобилке клик браузер отработает как тап)

// всего их 6
//1- touchstart  //событие возникает при касании к этому элементу
//2- touchmove   //событие при касании и движении. На каждое движение вызывается событие
//3- touchend    //событие когда отрываем палец от экрана
//4- touchenter  //когда палец зашел на пределы какого-то элемента
//5- touchleave  //когда палец вылез за пределы границ элемента
//6- touchcancel //когда точка больше не регистрируется на поверхности, например, когда палец выходит за пределы браузера
...
window.addEventListener('DOMContentLoaded', () =>{
	...
	box.addEventListener('touchstart', (e) =>{
		e.preventDefault();
		//для мобилок всегда рекомендуется передавать аргумент Ивент и прописывать превентДефаулт
		// чтобы всяких артефактов и нестыковок не было
	...
	// кстати на мобилка объект Ивент имеет дополнительные свойства и плюшки
} ); 

// touches  - список всех пальцев, которые сейчас взаимодействуют с экраном
// targetTouches  - все пальцы, что взаимодействуют с конкретным элементом
// changedTouches - все пальцы, что участвуют в текущем событии. Например, если событие тачЕнд и из 5 пальцев на экране убрали только один, то он и отобразится
//
// ТаргетТачес - это массив, т.е можно ТаргетТачес[0] и тд. Плюс можно отслеживать их координаты
///////
///
///
///
// Атрибут defer позволяет загружать страницу и продолжать загружать скрипт в фоновом режиме, а потом запустить как загрузится
// например,  <script defer src="js/script.js"></script>
// 1) defer не блокирует страницу. Например, код дошел до дефер и продолжая его загружать, читает стр дальше
// 2) и еще это выполняется когда ДОМ дерево уже готово
// 3) загрузка всех скриптов с атрибутом Дефер производится последовательно. Пока не загрузится верхний, не загрузится и нижний
//
//
// Атрибут async
// 1) Скрипты не ждут друг друга
// 2) Скрипты с таким атрибутом не ждут пока загрузится ДОМ 
// 3) Скрипт запускается как только был загружен. Н отут мы не будем знать кто в каком порядке. Будет просто: кто первый загрузился
//
//
//
// А еще мы можем создать скрипт в файле JS и затем поместить его на страницу, как мы это делали с отдельными элементами html
const script = document.createElement('script');
script.src = 'js/test.js';
document.body.append(script);
// и вот тут суть такова: скприпт добавляется после того как будет выполнен. ТО есть на 449 он только добавится из-за чего есть небольшая задержка в выполнении кода
// это является динамически загружаемым скриптом. Они ведут себя автоматически как async
// если мы не хотим чтобы скрипт был с атрибутом async то просто пишем
//script.async = false; тогда он будет вести себя как обычный скрипт без атрибутов
